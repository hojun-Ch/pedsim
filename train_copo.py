...
from eval_copo import eval_bypass, eval_crossing, eval_spread
...
if __name__ == '__main__':
    ...
    for j in range(args.max_step // args.rollout_length):
        ...
        for i in range(args.rollout_length):
            ...
            if i == 0:
                agent.rollout_buffer.add(feature, img, action, reward, n_reward, g_reward, np.array([1] * args.num_ped), value, n_value, g_value, log_prob, np.array([0.0] * args.num_ped))
            else:
                agent.rollout_buffer.add(feature, img, action, reward, n_reward, g_reward, np.array([0] * args.num_ped), value, n_value, g_value, log_prob, np.array([0.0] * args.num_ped))
        env.close()
        print("episode end!")
