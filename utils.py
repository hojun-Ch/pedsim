...
def obs_to_reward_coll_smoothed(obs, prev_obs, goal,  l, w, num_ped, coll_penalty, neighbor_distance):
    reward = np.zeros(num_ped)
    g_reward = np.ones(num_ped)
    n_reward = np.zeros(num_ped)
    neighbor_num = np.ones(num_ped)
    ...
    for i in range(num_ped):
        ...
        cost = 0
        min_distance = 1000
        all_x = obs[0][[5*k for k in range(num_ped)]]
        all_z = obs[0][[5*k + 1 for k in range(num_ped)]]
